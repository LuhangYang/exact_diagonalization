...
dt = np.dtype(np.float64)#np.dtype(np.complex128)
#L=10 #Cu2O8
L=9 #Cu2O7
...
#print("basis: ")
#dim,basis = build_basis_useQN(L,Nup,Ndn)
#print([bin(basis[i]) for i in range(dim)])
...
'''
##################### Measure Energy of a vector state #############################
def hami_energy(basis,basis_coef,L,all_term_list):
    E0 = 0.0
    basislen = len(basis)
    ...
    for term in all_term_list: 
        for i in range(basislen):
            ...
            if(state_out in basis):
                ...
                E0 += x0*basis_coef[i]*basis_coef[out_idx]
    return E0
...
def hami_mtx_block_2D(kx_total,ky_total,L,all_term_list,basis_set): #k_total=0,1,2,...L-1
    basislen = len(basis_set[ky_total][kx_total])
    print("length of the basis_ste:", len(basis_set),len(basis_set[0]))
    hmatrix = np.zeros(shape = (basislen,basislen),dtype=dt)
    #print("lenth of basis: ", basislen,[bin(basis_set[k_total][i]) for i in range(basislen)])
    termlen = len(all_term_list)
    for term in all_term_list: 
        #print(term.lst,term.coef)
        for i in range(basislen):
            state_out,x0 = hami(basis_set[ky_total][kx_total][i], term)
            if(state_out in basis_set[ky_total][kx_total]):
                #if(abs(x0)>1e-8): print(basis_set[k_total][i], term.lst,state_out,indexinlist(basis_set[k_total],state_out),x0)
                out_idx = indexinlist(basis_set[ky_total][kx_total],state_out)
                if(out_idx>=i):
                    hmatrix[i,out_idx] += x0
                    hmatrix[out_idx,i] = np.conjugate(hmatrix[i,out_idx])
    values,vectors = np.linalg.eigh(hmatrix)
    #print(hmatrix)
    return values,vectors, hmatrix

def hami_mtx_block_specific_basis(L,all_term_list,basis_s): #k_total=0,1,2,...L-1
    basislen = len(basis_s)
    print("length of the basis_ste:", len(basis_s))
    hmatrix = np.zeros(shape = (basislen,basislen),dtype=dt)
    #print("lenth of basis: ", basislen,[bin(basis_set[k_total][i]) for i in range(basislen)])
    termlen = len(all_term_list)
    for term in all_term_list: 
        for i in range(basislen):
            state_out,x0 = hami(basis_s[i], term)
            if(state_out in basis_s):
                #if(abs(x0)>1e-8): print(basis_set[k_total][i], term.lst,state_out,indexinlist(basis_set[k_total],state_out),x0)
                out_idx = indexinlist(basis_s,state_out)
                if(out_idx>=i):
                    hmatrix[i,out_idx] += x0
                    hmatrix[out_idx,i] = np.conjugate(hmatrix[i,out_idx])
    values,vectors = np.linalg.eigh(hmatrix)
...
'''
print("basis: ")
basis,Nf,dim,period = build_basis_k(L,Nup,Ndn)
print(basis)
print([bin(basis[i]) for i in range(dim)])


out_state, x = hami_k(201, 1,t,U)
print("check hami k: ")
print("instate: ",201,bin(201)," outstate: ",[out_state[xxx] for xxx in range(len(out_state))],[bin(out_state[xxx]) for xxx in range(len(out_state))],"x: ",[x[xxx] for xxx in range(len(out_state))])
'''
